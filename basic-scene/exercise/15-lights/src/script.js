...
// This example is from previous lessons
...
const pointLight = new THREE.PointLight(0xff9000, 0.5, 10, 2) // (color, intensity, distance, fade)
...
const rectAreaLight = new THREE.RectAreaLight(0x4e00ff, 2, 1, 1) // (colr, intensity, width, height)
rectAreaLight.position.set(-1.5, 0, 1.5)
rectAreaLight.lookAt(new THREE.Vector3())
scene.add(rectAreaLight)
gui.add(rectAreaLight, 'intensity').min(0).max(1).step(0.001)
gui.add(rectAreaLight, 'width').min(0.1).max(10).step(0.1)
gui.add(rectAreaLight, 'height').min(0.1).max(10).step(0.1)
...
gui.add(spotLight, 'distance').min(0.1).max(100).step(0.1)
gui.add(spotLight, 'angle').min(0).max(1).step(0.001)
gui.add(spotLight, 'penumbra').min(0).max(1).step(0.001)
gui.add(spotLight, 'decay').min(0.1).max(10).step(0.1)
